...
function bindColumnHover(tableId) {
  ...
  function setHover(idx) {
    ...
    if (lastIdx >= 0) table.querySelectorAll(`thead th:nth-child(${lastIdx+1}), tbody td:nth-child(${lastIdx+1})`).forEach(el => el.classList.remove("col-hover"));
    lastIdx = idx;
...
function buildDashHeader() {
  ...
  head.querySelectorAll("th").forEach(th => {
    th.addEventListener("click", () => {
      ...
      if (dashState.sortKey === key) dashState.sortDir = dashState.sortDir === "asc" ? "desc" : "asc";
      else { dashState.sortKey = key; dashState.sortDir = key === "Player" ? "asc" : "desc"; }
      store.set("dashSortKey", dashState.sortKey); store.set("dashSortDir", dashState.sortDir);
      renderDashboard();
    });
  });
}
function renderDashboard() {
  ...
  const rows = dashState.rows.slice(); const body = $("#dash-body"); const head = $("#dash-head");
  rows.sort((a,b) => {
...
function buildDetailHeader() {
  ...
  head.querySelectorAll("th").forEach(th => {
    th.addEventListener("click", () => {
      ...
      else { detailState.sortKey = key; detailState.sortDir = (key === "Date" || key === "Game") ? "desc" : "desc"; }
      store.set("detailSortKey", detailState.sortKey); store.set("detailSortDir", detailState.sortDir);
      renderDetail();
    });
  });
}
...
async function loadDetail() {
  try {
    ...
    if (!rows.length) { detailState.players = []; detailState.games = []; renderDetail(); return; }
...
async function adminLoadPlayers() {
  ...
  players.forEach(p => {
    ...
    lab.className = "pill"; lab.dataset.id = String(p.id); lab.textContent = `${p.icon} ${p.name}`;
  ...
  if (delSel) {
    delSel.innerHTML = "";
    players.forEach(p => { const opt = document.createElement("option"); opt.value = String(p.id); opt.textContent = `${p.icon} ${p.name}`; delSel.appendChild(opt); });
  }
  ...
  $("#btn-save-entry").onclick = async () => {
    ...
    alert("Saved!");
    $("#note").value = ""; $("#per-player").innerHTML = ""; $("#btn-save-entry").disabled = true; $$("#players-list .pill.on").forEach(el => el.classList.remove("on"));
...
async function adminLoadRecent() {
  try {
    ...
    const container = $("#recent-entries"); container.innerHTML = "";
    ...
    ids.forEach(eid => {
      ...
      const del = document.createElement("button"); del.className = "action-button"; del.textContent = "Delete";
      del.addEventListener("click", async () => {
        try {
          const resp = await fetch(`/api/entries/${eid}`, { method:"DELETE" });
          const data = await resp.json().catch(()=>({}));
          if (!resp.ok || !data.ok) throw 0;
          await adminLoadRecent(); await loadDashboard(); if ($("#view-detail").style.display !== "none") await loadDetail();
